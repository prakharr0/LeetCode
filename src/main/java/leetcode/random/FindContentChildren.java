 ...
 * assign the cookie j to the child i, and the child i will be content. Your goal is to maximize
...
public class FindContentChildren {
    ...
    /**
     * Sort the arrays. Then iterate over g and find the size such that size >= greed.
     * if we run out of sizes (cookies) return the count.
     */
    public int findContentChildren(int[] g, int[] s) {
        ...
        for (int greedSize: g) {
            ...
            while (si < s.length && s[si] < greedSize) {
                si++;
            }
